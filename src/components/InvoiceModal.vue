...
export default {
  ...
  created(){
    ...
    this.invoiceDate = new Date(this.invoiceDateUnix).toLocaleDateString("en-my",this.dateOptions);
    }
  ...
  methods:{
     ...mapMutations(["TOGGLE_INVOICE", "TOGGLE_MODAL","TOGGLE_EDIT_INVOICE"]),//commit mutation
    ...
    closeInvoice(){
      this.TOGGLE_INVOICE();
       if (this.editInvoice){
        this.TOGGLE_EDIT_INVOICE(); //this if condition is to fix the problem when want to add new invoice but in edit invoice
      }
    },
     addNewInvoiceItem() {
    ...
    //this funtion is send the date to the firebase
    async uploadInvoice() {
      ...
      this.GET_INVOICES(); //to show the new added invoice in home page without refresh
    ...
    submitForm() {
      ...
      this.uploadInvoice();//is a function that create new data in firebase
    },
 ...
 computed: {
   ...
   watch: {
     //auto calculate the payment due date based on the selection by using watch
     //the fucntion name is same as the v-model
    paymentTerms() {
      const futureDate = new Date();
      this.paymentDueDateUnix = futureDate.setDate(futureDate.getDate() + parseInt(this.paymentTerms));
      this.paymentDueDate = new Date(this.paymentDueDateUnix).toLocaleDateString("en-my", this.dateOptions);
    },
  },
