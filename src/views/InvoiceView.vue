...
export default {
  ...
  methods: {
    ...
    updateStatusToPaid(docId) {
      this.UPDATE_STATUS_TO_PAID(docId);//action name
    },
  ...
  watch: {
    editInvoice() {
      if (!this.editInvoice) {
        this.currentInvoice = this.currentInvoiceArray[0];//here is to auto update the data after updated without refresh by own self
